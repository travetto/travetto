...
${d.Section('Retargetting Proxy')}

In addition to file watching, the module also provides a core utiltity for hot reloading at runtime.  The framework makes use of ${d.Input('ES2015')} ${d.Input('Proxy')}s.  Specifically the the module provides ${RetargettingProxy}, as a means to provide a reference that can have it's underlying target changed at runtime. 

${d.Code(d`Example of using the ${RetargettingProxy.name}`, 'doc/proxy.ts')}
`;