...
export class AssertCheck {
  ...
  static check(assertion: CapturedAssertion, positive: boolean, ...args: unknown[]): void {
    ...
    try {
      ...
      if (error instanceof assert.AssertionError) {
        ...
        assertion.message = assertion.message.replace(/[{]([A-Za-z]+)[}]/g, (a, key: StringFields<Assertion>) => common[key] || assertion[key]!).replace(/not not/g, ''); // Handle double negatives
  ...
  static #onError(
    ...
    const resolvedError = (missed && error) ?? AssertCheck.checkError(shouldThrow, error);
    if (resolvedError) {
   ...
   */
  static checkThrow(assertion: CapturedAssertion, positive: boolean, action: Function, shouldThrow?: ThrowableError, message?: string): void {
    ...
    try {
      ...
      AssertCheck.#onError(positive, message, error, missed, shouldThrow, assertion);
    } finally {
      AssertCapture.add(assertion);
    }
  }
