...
const Response = d.SnippetLink('TravettoResponse', 'src/typings.d.ts', /interface TravettoResponse/);
...
${d.Execute('Custom application', 'trv', ['run', 'custom'], {
  cwd: './doc-exec'
})})}
...
${d.Ordered(
  ...
  ${d.Snippet('Cors Config', RestBodyParseConfig, /class.*Config/, /^\}/)}`,
  ...
  ${d.Snippet('Cors Config', RestCorsConfig, /class.*Config/, /^\}/)}`,
  ...
  ${d.Snippet('Cookies Config', RestCookieConfig, /class.*Config/, /^\}/)}`,
  ...
  ${d.Code('Control Logging', 'doc/log.yml')}`,
...
${d.List(
  d`Enable/disable of individual interceptors via configuration ${d.Code('Sample interceptor disabling configuration', 'doc/disable.yml')}`,
  d`Path-based control for various routes within the application ${d.Code('Sample interceptor path managed configuration', 'doc/route-allow-deny.yml')}`,
