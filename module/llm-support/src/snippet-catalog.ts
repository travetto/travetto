...
export async function recommendSnippets(query: RecommendationQuery = {}): Promise<SnippetSource[]> {
  ...
  const byOperation = query.operations && query.operations.length > 0 ? snippets.filter(item => matchesAny(item.operationIds ?? [], query.operations)) : snippets;

  const byTag = query.snippetTags && query.snippetTags.length > 0 ? byOperation.filter(item => matchesAny(item.capabilityTags, query.snippetTags)) : byOperation;
