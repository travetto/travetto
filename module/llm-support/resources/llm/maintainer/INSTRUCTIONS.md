...
- **Inline SQL Formatting**: Format inline SQL queries using standard practices: one line per main section (`SELECT`, `FROM`, `WHERE`, `GROUP BY`, `ORDER BY`, etc.). For optional query clauses (such as limit, offset, or sort), place each optional clause on its own line (e.g. `FROM <table>\n${optionalLimit}\n${optionalSort}` where `\n` is an actual newline in the template literal), rather than placing optional tokens immediately adjacent to each other on a single line.
