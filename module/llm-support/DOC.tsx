...
export const text = (
  <>
    ...
    The {d.module('LlmSupport')} module provides guided LLM assistance for Travetto projects. It helps you move from a target outcome to a
    concrete plan by combining bundle recommendations, workflow guidance, operation planning, and snippet selection.
    <c.Section title="Install">
      ...
      The module is designed to be used through {d.module('Cli')} commands, so once the package is available you can invoke the LLM support
      workflow with {d.input('trv')}.
    </c.Section>
    <c.Section title="Usage">
      ...
      operation-specific hints such as route, controller, service, model, and email naming.
    </c.Section>
    <c.Section title="What It Supports">
      The module covers the core assistant paths used by Travetto projects:
      <ul>
        <li>
          <strong>Project bootstrap</strong> - guided module and backend selection for a new application.
        </li>
        <li>
          <strong>Web</strong> - route, controller, service, interceptor, and client-oriented flows.
        </li>
        <li>
          <strong>Auth</strong> - session-backed identity and auth-web guided setup.
        </li>
        <li>
          <strong>Model</strong> - persistence, query, indexed, and backend selection guidance.
        </li>
        <li>
          <strong>Upload</strong> - direct upload and presigned URL support.
        </li>
        <li>
          <strong>Workflow</strong> - deployment-oriented GitHub workflow generation.
        </li>
        <li>
          <strong>Quality</strong> - linting and test suite setup.
        </li>
        <li>
          <strong>Email</strong> - templates, rendering, transport, preview, and send flows.
        </li>
        <li>
          <strong>Test</strong> - fixture and suite generation guidance.
        </li>
        <li>
          <strong>Config</strong> - configuration class and file generation.
        </li>
        <li>
          <strong>Cache</strong> - cache decorators and evictions workflows.
        </li>
      </ul>
      The available categories exposed by the CLI are {d.input('project')}, {d.input('web')}, {d.input('auth')}, {d.input('model')},{' '}
      {d.input('upload')}, {d.input('workflow')}, {d.input('quality')}, {d.input('email')}, {d.input('test')}, {d.input('config')}, and{' '}
      {d.input('cache')}.
    </c.Section>
    <c.Section title="Bundles And Workflows">
      ...
      The recommendation output also includes snippets that match the selected operations and capability tags, so the generated plan stays
      tied to reusable implementation patterns.
    </c.Section>
    <c.Section title="Command Options">
      ...
      <ul>
        ...
        <li>
          {d.input('--monorepo')} - when used with {d.input('project-bootstrap')}, generate a workspace root and {d.input('packages/app')}{' '}
          project layout.
        </li>
        <li>
          {d.input('--workspace-path')} - customize the monorepo app location (for example {d.input('packages/api')}).
        </li>
    ...
    <c.Section title="MCP Integration">
      ...
      Supported methods are {d.input('initialize')}, {d.input('tools/list')}, and {d.input('tools/call')}. Requests and responses are
      newline-delimited JSON-RPC 2.0 payloads.
      <c.Code
        title="MCP request examples"
        language="json"
        src={[
