...
export abstract class ModelIndexedSuite extends BaseModelSuite<ModelIndexedSupport> {
  ...
  async queryList() {
    ...
    const { items: arr } = await service.listPageByIndex(User3, userAgeIndex, { name: 'bob' });
    ...
    // @ts-expect-error
    await assert.rejects(() => service.listPageByIndex(User3, userAgeIndex, {}), IndexedFieldError);
  }
  ...
  async queryListNoSelectedKeys() {
    ...
    const { items: arr } = await service.listPageByIndex(User3, userAgeNoKeyIndex, {});
  ...
  async queryDeepList() {
    ...
    const { items: arr } = await service.listPageByIndex(User4, childAgeIndex, { child: { name: 'bob' } });
    ...
    // @ts-expect-error
    await assert.rejects(() => service.listPageByIndex(User4, childAgeIndex, {}), IndexedFieldError);
  }
  ...
  async queryComplexDateList() {
    ...
    const { items: arr } = await service.listPageByIndex(User4, nameCreatedIndex, { child: { name: 'bob' } });
    ...
    // @ts-expect-error
    await assert.rejects(() => service.listPageByIndex(User4, nameCreatedIndex, {}), IndexedFieldError);
  }
  ...
  async upsertByIndex() {
    ...
    const { items: arr2 } = await service.listPageByIndex(User3, userAgeIndex, { name: 'bob' });
    assert(arr2.length === 2);

    await service.deleteByIndex(User3, userAgeIndex, user1);

    const { items: arr3 } = await service.listPageByIndex(User3, userAgeIndex, { name: 'bob' });
  ...
  async paginateByIndex() {
    ...
    do {
      const page = await service.listPageByIndex(User3, userAgeIndex, { name: 'page' }, { limit, offset });
  ...
  async paginateByIndexReverse() {
    ...
    do {
      const page = await service.listPageByIndex(User3, userAgeReversedIndex, { name: 'page' }, { limit, offset });
      items.push(...page.items.map(u => u.color!));
      offset = page.nextOffset;
    } while (offset);
