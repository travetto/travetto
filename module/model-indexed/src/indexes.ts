 ...
 */
export function keyedIndex<T extends ModelType, K extends KeyedIndexSelection<T>>(
  cls: Class<T>,
  config: { name: string; key: K }
): KeyedIndex<T, K, {}> {
 ...
 */
export function uniqueIndex<T extends ModelType, K extends KeyedIndexSelection<T>>(
  cls: Class<T>,
  config: { name: string; key: K }
): KeyedIndex<T, K, {}> {
  const { name, key } = config;
 ...
 */
export function sortedIndex<T extends ModelType, K extends KeyedIndexSelection<T>, S extends SortedIndexSelection<T>>(
  cls: Class<T>,
  config: { name: string; key: K; sort: S }
): SortedIndex<T, K, S> {
