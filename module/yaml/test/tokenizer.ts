...
export class TokenizerTest {
  ...
  testJSON() {
    ...
    assert.deepStrictEqual(Tokenizer.cleanTokens(json), ['hello', ' ', '[1,2,3,4,5]']);
  }

  @Test()
  testNested() {
    const tokens = Tokenizer.tokenize('- - a: 4');
    assert.deepStrictEqual(tokens, ['-', ' ', '-', ' ', 'a', ':', ' ', '4']);
  }
