...
export abstract class WebUploadServerSuite extends BaseWebSuite {
  ...
  async testUploadAll() {
    ...
    assert(response.body?.hash === await EncodeUtil.hash(file, { hashAlgorithm: 'sha256' }));
  }
  ...
  async testMultiUpload() {
    ...
    const file = await this.fixture.readStream('/logo.png');
    const hash = await EncodeUtil.hash(file, { hashAlgorithm: 'sha256' });
  ...
  async testMultiUploadCustom() {
    ...
    const hash1 = await EncodeUtil.hash(file1, { hashAlgorithm: 'sha256' });

    const file2 = await this.fixture.readStream('/logo.png');
    const hash2 = await EncodeUtil.hash(file2, { hashAlgorithm: 'sha256' });
