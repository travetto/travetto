...
const InputSource = d.SnippetLink('InputSource', 'src/input/types.ts', /interface InputSource/);
...
The only provided ${InputSource} is the ${IterableInputSource} which supports all ${d.Input('Iterable')} and ${d.Input('Iterator')} sources.  Additionally, the module provides ${DynamicAsyncIterator} which allows for manual control of iteration, which is useful for event driven work loads.
