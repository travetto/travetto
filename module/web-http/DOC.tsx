...
export const text = (
  <>
    <c.StdHeader />
    This module provides basic for running {d.library('NodeHttp')}. {d.library('NodeHttps')} and {d.library('NodeHttp2')} servers, along
    with support for tls key generation during development.
    <c.CliHelpSection commandClass={WebHttpCommand}>
      By default, the framework provides a default {CliCommand} for {WebServerContract} that will follow default behaviors, and spin up the
      server.
    ...
    <c.Section title="Running a Server">
      <c.Execution
        ...
        cmd="trv"
        args={['web:http']}
        config={{ workingDirectory: './doc-exec', env: { TRV_ROLE: 'doc' } }}
      />
      ...
      <c.SubSection title="Creating a Custom CLI Entry Point">
        ...
        And using the pattern established in the {d.module('Cli')} module, you would run your program using{' '}
        {d.command(`${d.trv} web:custom`)}.
    ...
    <c.Section title="Node Web Http Server">
      ...
      Current the {NodeWebHttpServer} is the only provided {WebServerContract} implementation. It supports http/1.1, http/2, and tls, and is
      the same foundation as used by express, koa, and other popular frameworks.
    </c.Section>
    ...
    <c.Section title="TLS Support">
      Additionally the framework supports TLS out of the box, by allowing you to specify your public and private keys for the cert. In dev
      mode, the framework will also automatically generate a self-signed cert if:
      <ul>
      ...
      TLS support can be enabled by setting {d.input('web.http.tls: true')} in your config. The key/cert can be specified as string directly
      in the config file/environment variables. The key/cert can also be specified as a path to be picked up by {RuntimeResources}.
    </c.Section>
  </>
);
