...
import { TranspileUtil } from '../src/transpile';
...
export class TranspileUtilTest {
  ...
  resolveEnvToken() {
    process.env.NAME = '0';
    assert(TranspileUtil.resolveToken('-$NAME') === {
      minus: true,
      key: 'NAME',
      valid: true
    });
  ...
  @Test()
  resolveMacros() {
    const resolved = TranspileUtil.resolveMacros('name', `
...
let fsan = 'test'; // @line-if -fsa
    ...
    const resolvedFile = TranspileUtil.resolveMacros('name', `
// ${FILE_IF} fs
// ${FILE_IF} -fsa    
let fs   = 'test'; // @line-if fs 
let fsan = 'test'; // @line-if -fsa
    ...
    const resolvedFileNeg = TranspileUtil.resolveMacros('name', `
// ${FILE_IF} -fs
let fs   = 'test'; // @line-if fs 
let fsan = 'test'; // @line-if -fsa
        `);
  ...
  preProcess() {
    ...
    const results = TranspileUtil.preProcess('a', 'a\nb\nc\nHELLO');
