...
export class ManifestUtil {
  ...
  static #getNewest(stat: fs.Stats): number {
  ...
  static #transformFile(relative: string, full: string): ManifestModuleFile {
  ...
  static #describeModule({ name, folder }: Dependency): ManifestModule {
    const files = this.#scanFolder(folder, folder !== CWD ? new Set<string>(['src', 'bin', 'support']) : new Set<string>())
      .reduce<Record<string, ManifestModuleFile[]>>((acc, p) => {
        // Group by top folder
  ...
  static #buildManifestModules(): Record<string, ManifestModule> {
  ...
  static writeManifest(file: string, manifest: Manifest): void {
    let folder = file;
  ...
  static readManifest(file: string): Manifest | undefined {
    let folder = file;
    if (file.endsWith('.json')) {
      folder = path.dirname(file);
    } else {
  ...
  static #deltaModules(outputFolder: string, left: ManifestModule<DeltaModuleFiles>, right: ManifestModule<DeltaModuleFiles>): ManifestDeltaEvent[] {
  ...
  static produceDelta(outputFolder: string, left: Manifest, right: Manifest): Record<string, ManifestDeltaEvent[]> {
  ...
  static produceRelativeDelta(outputFolder: string, manifestFile: string): Record<string, ManifestDeltaEvent[]> {
