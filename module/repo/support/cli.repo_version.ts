...
export class RepoVersionCommand implements CliCommandShape {
  ...
  async main(level: SemverLevel, prefix?: string): Promise<void> {
    ...
    await PackageManager.version(RuntimeManifest, modules, level, prefix);
    ...
    if (this.commit) {
      ...
      await fs.utimes(path.resolve(RuntimeManifest.workspacePath, 'package.json'), Date.now(), Date.now());
    }
  }
}