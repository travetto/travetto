...
export class RepoVersionCommand implements CliCommandShape {
  ...
  async main(level: SemverLevel, prefix?: string): Promise<void> {
    ...
    await PackageManager.version(RuntimeIndex.manifest, modules, level, prefix);
    ...
    if (this.commit) {
      ...
      await fs.utimes(path.resolve(RuntimeIndex.manifest.workspacePath, 'package.json'), Date.now(), Date.now());
    }
  }
}