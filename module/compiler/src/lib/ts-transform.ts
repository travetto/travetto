    ...
    function visitNode(node: ts.Node) {
      ...
      if (originalNode && node.kind === ts.SyntaxKind.ExportDeclaration) {
        ...
        if (!!originalEd.exportClause !== !!ed.exportClause) {
          // Tsickle changes `* ...` into named exports.
    ...
    function visitNode(node: ts.Node) {
      if (node.kind === ts.SyntaxKind.Identifier) {
        ...
        if (parent1 && parent1.kind === ts.SyntaxKind.PropertyDeclaration) {
          ...
          (ts.getCombinedModifierFlags(parent3) & ts.ModifierFlags.Export) > 0) {
      ...
      // TypeScript ignores synthetic comments on re/ import statements.
      ...
      if (moduleName && fileContext.importOrReexportDeclarations) {
        // Locate the original import/declaration via the
        // text range.
 ...
 */
function visitNodeWithSynthesizedComments<T extends ts.Node>(
 ...
 */
function visitEachChildIgnoringTypes<T extends ts.Node>(
...
function isTypeNodeKind(kind: ts.SyntaxKind) {
