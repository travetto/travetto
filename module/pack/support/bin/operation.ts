...
export class PackOperation {
  ...
  static async * bundle(config: CommonPackConfig): AsyncIterable<string[]> {
    ...
    const bundleCommand = [Runtime.packageCommand('rollup'), '-c', RuntimeIndex.resolveFileImport(config.rollupConfiguration)];
  ...
  static async * writeManifest(config: CommonPackConfig): AsyncIterable<string[]> {
    ...
    const cmd = [Runtime.packageCommand('trvc'), 'manifest:production', out];
