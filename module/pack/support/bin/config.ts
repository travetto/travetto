...
import { ManifestModule, ManifestModuleUtil, NodeModuleType, path, RuntimeIndex, RuntimeManifest } from '@travetto/manifest';
...
export function getOutput(): OutputOptions {
  ...
  return {
    ...
    sourcemapPathTransform: (src, map): string =>
      path.resolve(path.dirname(map), src).replace(`${RuntimeManifest.workspacePath}/`, ''),
