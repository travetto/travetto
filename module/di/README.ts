import { d, Library, inp, Ref, Section, Code, Note, SubSection } from '@travetto/doc';
import { Injectable, InjectableFactory, Inject } from './src/decorator';
import { Config } from '@travetto/config';
import { $DependencyRegistry } from './src/registry';

export default d`
${Library(`Dependency injection`, 'https://en.wikipedia.org/wiki/Dependency_injection')} is a framework primitive.  When used in conjunction with automatic file scanning, it provides for handling of application dependency wiring. Due to the nature of ${Library(`typescript`, 'https://typescriptlang.org')} and type erasure of interfaces, dependency injection only supports ${inp`class`}es as type signafiers. The primary goal of dependency injection is to allow for separation of concerns of object creation and it's usage.
...
${Code(d`Example ${Injectable}`, 'alt/docs/src/injectable.ts')}
...
${Code(d`Example ${Injectable} with multiple targets`, 'alt/docs/src/injectable-multiple.ts')}
...
${Code(d`Example ${Injectable} with target via abstract class`, 'alt/docs/src/injectable-abstract.ts')}
...
${Code(d`Example ${InjectableFactory}`, 'alt/docs/src/injectable-factory.ts')}
...
${Code(d`Example ${Injectable} with dependencies as ${Inject} fields`, 'alt/docs/src/injectable-fields.ts')}


The ${Injectable} constructor params, which will be provided as the instance is being constructed.

${Code(d`Example ${Injectable}  with dependencies in constructor`, 'alt/docs/src/injectable-constructor.ts')}

Via ${InjectableFactory} params, which are comparable to constructor params

${Code(d`Example ${InjectableFactory} with parameters as dependencies`, 'alt/docs/src/injectable-factory-params.ts')}
