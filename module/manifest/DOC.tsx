...
const manifest = async () => {
  ...
  return JSON.stringify({ ...result, modules }, null, 2).replace(/\[[^[]*?\]/gms, value => value.replace(/\s+/gs, ' '));
};
  ...
  <>
    ...
    <c.Section title="Project Manifesting">
      ...
      <c.SubSection title="Compile-time Support">
        During the compilation process, the compiler needs to know every file that is eligible for compilation, when the file was last
        created/modified, and any specific patterns for interacting with a given file (e.g. transformers vs. testing code vs. support files
        that happen to share a common extension with code).
      </c.SubSection>
      <c.SubSection title="Runtime Knowledge">
        Additionally, once the code has been compiled (or even bundled after that), the executing process needs to know what files are
        available for loading, and any patterns necessary for knowing which files to load versus which ones to ignore. This allows for
        dynamic loading of modules/files without knowledge/access to the file system, and in a more performant manner.
      </c.SubSection>
    </c.Section>
    <c.Section title="Manifest Delta">
      During the compilation process, it is helpful to know how the output content differs from the manifest, which is produced from the
      source input. The {DeltaRef} provides the functionality for a given manifest, and will produce a stream of changes grouped by module.
      This is the primary input into the {d.module('Compiler')}'s incremental behavior to know when a file has changed and needs to be
      recompiled.
    </c.Section>
    <c.Section title="Module Indexing">
      Once the manifest is created, the application runtime can now read this manifest, which allows for influencing runtime behavior. The
    ...
    <c.Section title="Path Normalization">
      ...
      This works appropriately within a Unix and a Windows environment. This module offers up{' '}
      <c.CodeLink title="path" src="./src/path.ts" startRe={/export/} /> as an equivalent to {d.library('Node')}'s {d.library('NodePath')}{' '}
      library. This allows for consistent behavior across all file-interactions.
    </c.Section>
    <c.Section title="Anatomy of a Manifest">
      ...
      <c.SubSection title="General Context">
        ...
        <ul>
          ...
          <li>
            ...
            <em>This primarily pertains to mono-repo support when there are multiple modules in the project</em>)
          </li>
          <li>The root path of the project/workspace</li>
          <li>
            Whether or not the project is a mono-repo. (
            <em>This is determined by using the 'workspaces' field in your {d.library('PackageJson')}</em>)
          </li>
          <li>
            The location where all compiled code will be stored. Defaults to: {d.path('.trv_output')}. (
            <em>Can be overridden in your {d.library('PackageJson')} in 'travetto.outputFolder'</em>)
          </li>
      ...
      <c.SubSection title="Modules">
        The modules represent all of the {d.library('Travetto')}-aware dependencies (including dev dependencies) used for compiling, testing
        and executing. A production-only version is produced when packaging the final output. Each module contains:
        <ul>
      ...
      <c.SubSection title="Module Files">
        ...
        <ul>
          ...
          <li>
            {d.path('$index')} - {d.path('__index__.ts')}, {d.path('index.ts')} files at the root of the project
          </li>
          <li>
            {d.path('$package')} - The {d.library('PackageJson')} for the project
          </li>
          <li>
            {d.path('src')} - Code that should be automatically loaded at runtime. All .ts files under the {d.path('src/')} folder
          </li>
          <li>
            {d.path('test')} - Code that contains test files. All .ts files under the {d.path('test/')} folder
          </li>
          <li>
            {d.path('test/fixtures')} - Test resource files, pertains to the main module only. Located under {d.path('test/fixtures/')}
          </li>
          <li>
            {d.path('resources')} - Packaged resource, meant to pertain to the main module only. Files, under {d.path('resources/')}
          </li>
          <li>
            {d.path('support')} - All .ts files under the {d.path('support/')} folder
          </li>
          <li>
            {d.path('support/resources')} - Packaged resource files, meant to be included by other modules, under{' '}
            {d.path('support/resources/')}
          </li>
          <li>
            {d.path('support/fixtures')} - Test resources meant to shared across modules. Under {d.path('support/fixtures/')}
          </li>
          <li>
            {d.path('doc')} - Documentation files. {d.path('DOC.tsx')} and All .ts/.tsx files under the {d.path('doc/')} folder
          </li>
          <li>
            {d.path('$transformer')} - All .ts files under the pattern {d.path('support/transform*')}. These are used during compilation and
            never at runtime
          </li>
          <li>
            {d.path('bin')} - Entry point .js files. All .js files under the {d.path('bin/')} folder
          </li>
        </ul>
