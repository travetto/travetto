 ...
 */
@Injectable()
export class MysqlModelService
  implements
    ModelCrudSupport,
    ModelStorageSupport,
    ModelBulkSupport,
    ...
    SQLDialect
{
  ...
  async upsertTable(modelClass: Class<ModelType>): Promise<void> {
    ...
    const tableExists = Number(tableCheck.records[0]?.total ?? 0) > 0;
    ...
    if (!tableExists) {
      ...
      for (const field of context.complexFields.values()) {
        columnDefinitions.push(`${this.escapeIdentifier(field.name)} JSON`);
      }
      ...
      for (const field of context.complexFields.values()) {
        requestedFieldsMap.set(field.name, 'JSON');
      }
      ...
      for (const indexName of existingIndexes) {
        if (!requestedIndexesMap.has(indexName)) {
          await this.connection.execute(`DROP INDEX ${this.escapeIdentifier(indexName)} ON ${this.escapeIdentifier(context.tableName)};`);
        }
      }
    }
  }
