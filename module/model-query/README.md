...
<!-- Please modify https://github.com/travetto/travetto/tree/main/module/model-query/README.ts and execute "npx trv doc" to rebuild -->
# Data Model Querying
...
|-------|-----|---------|----------|
|[Elasticsearch Model Source](module/model-elasticsearch#readme "Elasticsearch backing for the travetto model module, with real-time modeling support for Elasticsearch mappings.")|X|X|X|
|[MongoDB Model Support](module/model-mongo#readme "Mongo backing for the travetto model module.")|X|X|X|
|[SQL Model Service](module/model-sql#readme "SQL backing for the travetto model module, with real-time modeling support for SQL schemas.")|X|X|X|
...
repetitive functionality, that is unable to be shared due to not relying upon inheritance (this was an intentional design decision).  This allows for all the [Data Model Querying](module/model-query#readme "Datastore abstraction for advanced query support.") implementations to completely own the functionality and also to be able to provide additional/unique functionality that goes beyond the interface.
