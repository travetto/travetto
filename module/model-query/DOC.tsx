...
export const text = (
  <>
    <c.StdHeader />
    This module provides an enhanced query contract for {d.module('Model')} implementations. This contract has been externalized due to it
    being more complex than many implementations can natively support.
    <c.Section title="Contracts">
      <c.SubSection title="Query">
        This contract provides the ability to apply the query support to return one or many items, as well as providing counts against a
        specific query.
        <c.Code title="Query" src={toConcrete<ModelQuerySupport>()} />
      </c.SubSection>
      <c.SubSection title="Crud">
        Reinforcing the complexity provided in these contracts, the {Links.QueryCrud} contract allows for bulk update/deletion by query.
        This requires the underlying implementation to support these operations.
        <c.Code title="Query Crud" src={toConcrete<ModelQueryCrudSupport>()} />
      </c.SubSection>

      <c.SubSection title="Facet">
        With the complex nature of the query support, the ability to find counts by groups is a common and desirable pattern. This contract
        allows for faceting on a given field, with query filtering.
        <c.Code title="Query Facet" src={toConcrete<ModelQueryFacetSupport>()} />
      </c.SubSection>
      <c.SubSection title="Suggest">
        Additionally, this same pattern avails it self in a set of suggestion methods that allow for powering auto completion and type-ahead
        functionalities.
    ...
    <c.Section title="Querying">
      One of the complexities of abstracting multiple storage mechanisms, is providing a consistent query language. The query language the
      module uses is a derivation of {d.library('MongoDB')}'s query language, with some restrictions, additions, and caveats. Additionally,
      given the nature of typescript, all queries are statically typed, and will catch type errors at compile time.
      ...
      <c.SubSection title="General Single Valued Fields">
        <ul>
          <li>
            {d.input('field: { $in: T[] }')} to see if a record's value appears in the array provided to {d.input('$in')}
          </li>
          <li>
            {d.input('field: { $nin: T[] }')} to see if a record's value does not appear in the array provided to {d.input('$in')}
          </li>
        </ul>
      </c.SubSection>
      ...
      <c.SubSection title="Ordered Date Fields">
        ...
        <c.Note>
          Relative times are strings consisting of a number and a unit. e.g. -1w or 30d. These times are always relative to Date.now, but
          should make building queries more natural.
        </c.Note>
      </c.SubSection>
      <c.SubSection title="Array Fields">
        <ul>
          <li>
            {d.input('field: { $all: T[]] }')} checks to see if the records value contains everything within {d.input('$all')}
          </li>
        </ul>
      </c.SubSection>
      ...
      <c.SubSection title="Geo Point Fields">
        <ul>
          ...
          <li>
            {d.input("field: { $near: Point, $maxDistance: number, $unit: 'km' | 'm' | 'mi' | 'ft' }")} searches at a point, and looks out
            radially
          </li>
        </ul>
      </c.SubSection>
    ...
    <c.Section title="Custom Model Query Service">
      ...
      decision).This allows for all the {d.module('ModelQuery')} implementations to completely own the functionality and also to be able to
      provide additional / unique functionality that goes beyond the interface. To enforce that these contracts are honored, the module
      provides shared test suites to allow for custom implementations to ensure they are adhering to the contract's expected behavior.
