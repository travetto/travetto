...
import { BinaryUtil, type ByteArray } from '@travetto/runtime';
...
export class EtagInterceptor implements WebInterceptor {
  ...
  computeTag(body: ByteArray): string {
    return body.byteLength === 0 ?
      '2jmj7l5rSw0yVb/vlWAYkK/YBwk' :
      crypto
        .createHash('sha1')
        .update(BinaryUtil.toUTF8String(body), 'utf8')
        .digest('base64')
        .substring(0, 27);
  }
  ...
  addTag(ctx: WebChainedContext<EtagConfig>, response: WebResponse): WebResponse {
    ...
    if (!BinaryUtil.isByteArray(body)) {
