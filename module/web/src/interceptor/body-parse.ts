...
import { HttpRequest, HttpContext, WebInternal, NextFilter } from '../types.ts';
...
export class BodyParseInterceptor implements HttpInterceptor<BodyParseConfig> {
  ...
  async filter({ req, config }: HttpContext<BodyParseConfig>, next: NextFilter): Promise<unknown> {
