...
import { HttpRequest, HttpContext, WebInternal, NextFunction } from '../types.ts';
...
export class BodyParseInterceptor implements HttpInterceptor<BodyParseConfig> {
  ...
  async filter({ req, config }: HttpContext<BodyParseConfig>, next: NextFunction): Promise<unknown> {
