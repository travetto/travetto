...
export class EndpointParameterTest {
  ...
  async testOptional() {
    ...
    await assert.rejects(() => EndpointParameterTest.extract(ep, {}), ValidationResultError);
  }
  ...
  async testArray() {
    ...
    assert.deepStrictEqual(
      await EndpointParameterTest.extract(ep2, {
        context: {
          path: '/array',
          httpQuery: { values: 'no' }
        }
  ...
  async realWorldUserInterface() {
    ...
    const extracted = await EndpointParameterTest.extract(ep, {
      context: {
        path: '/interface-prefix',
