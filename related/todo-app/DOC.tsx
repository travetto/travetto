...
import { c, type DocJSXElement, type DocJSXElementByFn, DocRunUtil, d, isDocJSXElement, PackageDocUtil } from '@travetto/doc';
...
function TableOfContents({ root }: { root: () => DocJSXElement }) {
  ...
  const sections: DocJSXElementByFn<'Section'>[] = final.filter(
    x => isDocJSXElement(x) && x.type === c.Section && castTo<DocJSXElementByFn<'Section'>>(x).props.title !== 'Overview'
  );
  return (
    <ol>
      {...sections.map(item => (
        <li>
          <c.Anchor title={item.props.title} href={item.props.title} />
        </li>
      ))}
    </ol>
  );
...
export const text = async () => {
  ...
  return (root = (
    <>
      ...
      The following tutorial wil walk you through setting up a {d.library('Travetto')} application from scratch. We'll be building a simple
      todo application. The entire source of the finished project can be found at {TodoRoot}. If you want the LLM-assisted path instead of
      manual setup, start with {d.input('trv llm-support:recommend')} to pick a bundle, {d.input('trv llm-support:plan')} to preview the
      changes, and {d.input('trv llm-support:execute')} to apply them.
...
`}
      ...
      <c.Section title="Establishing The Model">
        ...
        as you can see, the model structure is simple. Everything that uses the {Model} services needs to implement{' '}
        {toConcrete<ModelType>()}.
      </c.Section>
      ...
      <c.Section title="Writing Unit tests">
        After we have established our service layer, we will now construct some simple tests to verify the service layer is running
        correctly. By default we set the database schema name under {d.path('test/resources/application.yml')} to ensure we aren't writing
        to our dev database.
      ...
      <c.Section title="Running the App">
        ...
        <c.Terminal title="Start the Application" src={`${d.trv} web:http`} />
        ...
        <c.Execution
          title="Create Output"
          cmd="trv"
          args={['main', 'doc/create-todo.ts', key, `${port}`]}
          config={{
            ...
            formatCommand: (name, args) =>
              [name, ...args]
                .map(arg =>
                  arg
                    .replaceAll('doc/create', 'support/create')
                    .replace(key, '<key>')
        ...
        <c.Execution
          ...
          cmd="trv"
          args={['main', 'doc/list-todo.ts', key, `${port}`]}
          config={{
            rewrite: line => line.replaceAll(key, '<key>').replace(/[0-9a-f]{32}/, '<uniqueId>'),
            formatCommand: (name, args) =>
              [name, ...args]
                .map(arg =>
                  arg
                    .replaceAll('doc/list', 'support/list')
                    .replace(key, '<key>')
                    .replace(new RegExp(`${port}`, 'g'), '<port>')
                )
                .join(' ')
