...
import { ExecUtil, Env, Util } from '@travetto/runtime';
...
export class CliRunUtil {
  ...
  static async getModules(): Promise<ModuleGraphItem<Set<string>>[]> {
    ...
    try {
      const result: ModuleGraphItem<string[]>[] = Util.parseJSONSafe(data.stdout);
  ...
  static async getChoices(): Promise<RunChoice[]> {
    ...
    let choices: RunChoice[] = Util.parseJSONSafe(data.stdout);
